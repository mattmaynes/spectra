...
echo "10. What's new marker pair present exactly once"
wnstart='<!-- whats-new:start -->'; wnend='<!-- whats-new:end -->'
# The block whats-new.sh rewrites must exist exactly once, or a future README edit that drops a
...
# whats-new.sh itself is owned by the Trellis plugin-release template and unit-tested there.

echo "11. version in sync across all manifests (plugin-release template)"
# bump-version.sh + release.yml + whats-new.yml come from the Trellis plugin-release template
# (docs/rules/.trellis-owned-plugin-release); VERSION is the source of truth and --check is the
# CI drift guard.
if sh "$ROOT/scripts/bump-version.sh" --check >/dev/null 2>&1; then
  ok "bump-version --check passes (VERSION matches every manifest in .version-manifests)"
else
  bad "manifests drift from VERSION - run scripts/bump-version.sh \$(cat VERSION)"
fi
